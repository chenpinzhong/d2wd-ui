<?php
	...
	*/
	...
	class bbs_catalog extends controller{
		...
		public function index(){
			$catalog_array=$this->get_bbs_catalog();
			$this->assign("catalog_json",json_encode($catalog_array,true));//设置语言参数
		}
		
		//获取类型所有信息
		public function get_bbs_catalog($pid=0){
			$mdb=$this->mdb;
			$catalog_sqlstr="SELECT * FROM ts_bbs_catalog where pid=".$mdb->qstr($pid);
			$catalog_sql=$mdb->query($catalog_sqlstr);
			$catalog_array=array();
			while($catalog_return=$mdb->fetch_assoc($catalog_sql)){
				$bbs_catalog_id=$catalog_return['id'];
				$catalog_pid=$catalog_return['pid'];
				
				//查询类目名称
				$catalog_name_sqlstr="SELECT * FROM ts_bbs_catalog_name where bbs_catalog_id=".$mdb->qstr($bbs_catalog_id);
				...
				$catalog_return['child']=$this->get_bbs_catalog($bbs_catalog_id);
		...
		//获取类目下所有的类目id
		public function get_bbs_catalog_id($pid=0){
			$mdb=$this->mdb;
			...
			while($catalog_return=$mdb->fetch_assoc($catalog_sql)){
				...
				$catalog_array=array_merge($catalog_array,$this->get_bbs_catalog_id($bbs_catalog_id));//合并子集的id信息到数组
			}
			return $catalog_array;
		}
		
		//添加信息类目
		public function add_bbs_catalog(){
			$mdb=$this->mdb;
			...
			$catalog_sqlstr="SELECT * FROM ts_bbs_catalog where 1=1 ";
			if(is_numeric($id)){
			...
			if(empty($catalog_rows)){
				//插入
				$bbs_catalog_id=$mdb->insert('ts_bbs_catalog',$catalog_data);
			}else{
				//更新
				$bbs_catalog_id=$catalog_rows['id'];
				$mdb->update('ts_bbs_catalog',$catalog_data,array('id'=>$bbs_catalog_id));
			}
			...
			foreach($category_name_array as $language_type=>$catalog_name){
				//查询产品是否存在
				$catalog_name_rows=$mdb->table('ts_bbs_catalog_name')->where(array('language_type'=>$language_type,'bbs_catalog_id'=>$bbs_catalog_id))->find();
				
				$catalog_name_data=array();
				$catalog_name_data['bbs_catalog_id']=$bbs_catalog_id;
				...
				if(empty($catalog_name_rows)){
					...
					$mdb->insert('ts_bbs_catalog_name',$catalog_name_data);//不需要获取 product_name_id
				}else{
					$catalog_name_id=$catalog_name_rows['id'];
					$mdb->update('ts_bbs_catalog_name',$catalog_name_data,$catalog_name_id);
				}
				
			}
			
			$result=array('status'=>200,'bbs_catalog_id'=>$bbs_catalog_id);
			echo (json_encode($result));
		}
		
		//修改类目
		public function modify_bbs_catalog(){
			$mdb=$this->mdb;
			$request=$this->request;
			...
			$catalog_sqlstr="SELECT * FROM ts_bbs_catalog where id=".$mdb->qstr($id);
			$catalog_rows=$mdb->find($catalog_sqlstr);
		...
		//删除类目
		public function delete_bbs_catalog(){
			$mdb=$this->mdb;
			$request=$this->request;
			...
			if(empty($catalog_rows)){
				...
				$status=200;//pid 不存在 已经被输出
				//判断子集是否存在
				$catalog_id_array=$this->get_bbs_catalog_id($id);
				$catalog_id_array[]=$id;//也删除当前要删除的元素
				foreach($catalog_id_array as $catalog_id){
					$mdb->delete('ts_bbs_catalog',array('id'=>$catalog_id));
				}
			}
